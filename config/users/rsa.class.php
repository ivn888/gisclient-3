...
/* 
---- test.php ----- 
 require_once("$DOCUMENT_ROOT/security/rsa/rsa.class"); 

 $rsa = New SecurityRSA; 
 $keys = $rsa->generate_keys(1); 
 $message=""; 
 for ($i=32;$i<127;$i++) $message.=chr($i); 

 $encoded = $rsa->rsa_encrypt($message, $keys[1], $keys[0]); 
 $decoded = $rsa->rsa_decrypt($encoded, $keys[2], $keys[0]); 
