...
//  ----------------------------------------------------------------------------
//
// Description : This is a PHP5 class for generating PDF 
//               documents without requiring external 
//               extensions.
//
// NOTE:
...
/**
 * This is a PHP5 class for generating PDF documents without requiring external extensions.<br>
 * TCPDF project (http://www.tcpdf.org) was originally derived in 2002 from the Public Domain FPDF class by Olivier Plathey (http://www.fpdf.org).<br>
 * <h3>TCPDF main features are:</h3>
 * <ul>
 * <li>no external libraries are required for the basic functions;
 * <li>supports all ISO page formats;
 * <li>supports UTF-8 Unicode and Right-To-Left languages;
 * <li>supports document encryption;
 * <li>includes methods to publish some HTML code;
 * <li>includes graphic and transformation methods;
 * <li>includes bookmarks;
 * <li>includes Javascript and forms support;
 * <li>includes a method to print various barcode formats (requires GD library);
 * <li>supports TrueTypeUnicode, TrueType, Type1 and encoding;
 * <li>supports custom page formats, margins and units of measure;
 * <li>includes methods for page header and footer management;
 * <li>supports automatic page break;
 * <li>supports automatic page numbering;
 * <li>supports automatic line break and text justification;
 * <li>supports JPEG, PNG anf GIF images;
 * <li>supports colors;
 * <li>supports links;
 * <li>support page compression (requires zlib extension).
 * </ul>
 ...
 */
require_once ROOT_PATH.'config/config.tcpdf.php';

if(!class_exists('TCPDF', false)) {
    /**
     * define default PDF document producer
     */ 
    define('PDF_PRODUCER', 'TCPDF 3.0.003 (http://www.tcpdf.org)');
    ...
    class TCPDF        {
        ...
        */
        ...
        public function __construct($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding="UTF-8") 
        {
            ...
            case 'pt': {$this->k = 1; break;
             ...
             // millimeters
            case 'mm': {$this->k = $this->dpi / 25.4; break;
                }
             // centimeters
            case 'cm': {$this->k = $this->dpi / 2.54; break;
                }
             // inches
            case 'in': {$this->k = $this->dpi; break;
                }
             // unsupported unit
        ...
        */
        public function AddPage($orientation='') 
        {
            ...
            $style=$this->FontStyle.($this->underline ? 'U' : '').($this->linethrough ? 'D' : '');
        ...
        */
        public function SetFillColor($col1=0, $col2=-1, $col3=-1, $storeprev=false, $col4=-1) 
        {
            ...
            if (($col2==-1) AND ($col3==-1) AND ($col4==-1)) {
                ...
                if ($storeprev) {
                    array_push($this->bgcolor, array('G' => $col1));
                }
            } elseif ($col4==-1) {
                // RGB
                $this->FillColor = sprintf('%.3f %.3f %.3f rg', $col1/255, $col2/255, $col3/255);
                if ($storeprev) {
                    array_push($this->bgcolor, array('R' => $col1, 'G' => $col2, 'B' => $col3));
                }
            } else {
                // CMYK
                $this->FillColor = sprintf('%.3f %.3f %.3f %.3f k', $col1/100, $col2/100, $col3/100, $col4/100);
                if ($storeprev) {
                    array_push($this->bgcolor, array('C' => $col1, 'M' => $col2, 'Y' => $col3, 'K' => $col4));
                }
            }
        ...
        */
        public function SetTextColor($col1=0, $col2=-1, $col3=-1, $storeprev=false, $col4=-1) 
        {
            //Set color for text
            if (($col2==-1) AND ($col3==-1) AND ($col4==-1)) {
                // Grey scale
                ...
                if ($storeprev) {
                    array_push($this->fgcolor, array('G' => $col1));
                }
            } elseif ($col4==-1) {
                // RGB
                $this->TextColor = sprintf('%.3f %.3f %.3f rg', $col1/255, $col2/255, $col3/255);
                if ($storeprev) {
                    array_push($this->fgcolor, array('R' => $col1, 'G' => $col2, 'B' => $col3));
                }
            } else {
                // CMYK
                $this->TextColor = sprintf('%.3f %.3f %.3f %.3f k', $col1/100, $col2/100, $col3/100, $col4/100);
                if ($storeprev) {
                    array_push($this->fgcolor, array('C' => $col1, 'M' => $col2, 'Y' => $col3, 'K' => $col4));
                }
            }
            $this->ColorFlag = ($this->FillColor != $this->TextColor);
        }
        ...
        */
        public function GetArrStringWidth($sa) 
        {
            $w = 0;
            foreach($sa as $char) {
      ...
      *
        ...
        */
        public function Text($x, $y, $txt, $stroke=0, $clip=false) 
        {
            ...
            $s = sprintf('BT %.2f %.2f Td %s(%s) Tj ET', $xr * $this->k, ($this->h-$y) * $this->k, $opt, $this->_escapetext($txt));
      ...
      *
        ...
        * @param int    $ln      Indicates where the current position should go after the call. Possible values are:<ul><li>0: to the right (or left for RTL languages)</li><li>1: to the beginning of the next line</li><li>2: below</li></ul> Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0. Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
        ...
        */
        public function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=0, $link='', $stretch=0) 
        {
            ...
            $min_cell_height = $this->FontSize * K_CELL_HEIGHT_RATIO;
            ...
            if($txt != '') {
                ...
                if ($align == 'J') {
                    ...
                    if ($this->isunicode) {
                        ...
                        $this->_out(sprintf('BT %.3f Tw ET', $spacewidth));
                    }
                }
        ...
        * @param int     $stretch stretch carachter mode: <ul><li>0 = disabled</li><li>1 = horizontal scaling only if necessary</li><li>2 = forced horizontal scaling</li><li>3 = character spacing only if necessary</li><li>4 = forced character spacing</li></ul>
        * @return int Return the number of cells.
        * @since 1.5
        */
        ...
        * @see AddLink()
        */
        public function Image($file, $x, $y, $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300) 
        {
            // get image size
            ...
            if(!isset($this->images[$file])) {
                ...
                // Check for GD image handler function
        ...
        * @param image  $image Image object. return image JPEG image object.
      ...
      *
        ...
        {
            include_once dirname(__FILE__)."/barcode/barcode.php";
            include_once dirname(__FILE__)."/barcode/i25object.php";
            include_once dirname(__FILE__)."/barcode/c39object.php";
            include_once dirname(__FILE__)."/barcode/c128aobject.php";
            include_once dirname(__FILE__)."/barcode/c128bobject.php";
            include_once dirname(__FILE__)."/barcode/c128cobject.php";
                ...
                if (!preg_match($pattern, $element)) {
                    ...
                    if($this->HREF) {
                        ...
                        if((strlen(trim($element)) > 0) AND ($element != "&nbsp;")) {
                            $this->Cell($this->tdwidth, $this->tdheight, $this->unhtmlentities($element), $this->tableborder, '', $this->tdalign, $cfill);
                        } elseif($element == "&nbsp;") {
                            $this->Cell($this->tdwidth, $this->tdheight, '', $this->tableborder, '', $this->tdalign, $cfill);
                        }
                    } else {
         ...
         * @param mixed   $border Indicates if borders must be drawn around the cell. The value can be either a number:<ul><li>0: no border (default)</li><li>1: frame</li></ul>or a string containing some or all of the following characters (in any order):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul>
         * @param int     $ln     Indicates where the current position should go after the call. Possible values are:<ul><li>0: to the right (or left for RTL language)</li><li>1: to the beginning of the next line</li><li>2: below</li></ul> Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0. Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
    Putting 1 is equivalent to putting 0 and calling Ln() just after. Default value: 0.
         * @param int     $fill   Indicates if the cell background must be painted (1) or transparent (0). Default value: 0.
         * @param boolean $reseth if true reset the last cell height (default true).
         * @uses MultiCell()
         ...
         */
        public function writeHTMLCell($w, $h, $x, $y, $html='', $border=0, $ln=0, $fill=0, $reseth=true) 
        {
         ...
         * @access protected
         */
        protected function openHTMLTagHandler($tag, $attr, $fill=0) 
        {
            ...
            if (($tag != 'br') AND ($tag != 'hr') AND ($tag != 'img') ) {
                ...
                $this->SetTextColor($col['R'], $col['G'], $col['B'], true);
         ...
         * @access protected
         */
        protected function closedHTMLTagHandler($tag) 
        {
            // restore foreground color
            $nfg = count($this->fgcolor);
            if ($nfg > 1) {
                array_pop($this->fgcolor);
                $this->SetTextColor($this->fgcolor[$nfg-2]['R'], $this->fgcolor[$nfg-2]['G'], $this->fgcolor[$nfg-2]['B'], false);
            }
        ...
        /**
         * Sets font style.
         * @param string  $tag    tag name in lowercase. Supported tags are:<ul> <li>b : bold text</li> <li>i : italic</li> <li>u : underlined</li> <li>lt : line-through</li></ul> <li>b : bold text</li> <li>i : italic</li> <li>u : underlined</li> <li>lt : line-through</li></ul>
         * <li>b : bold text</li>
         * <li>i : italic</li>
         * <li>u : underlined</li>
        ...
        public function addHtmlLink($url, $name, $fill=0) 
        {
            ...
            $this->SetTextColor($prevcolor['R'], $prevcolor['G'], $prevcolor['B'], false);
        }
        ...
        * </ul>
        * @param array  $border_style Border style of rectangle. Array with keys among the following: <ul> <li>all: Line style of all borders. Array like for {@link SetLineStyle SetLineStyle}.</li> <li>L, T, R, B or combinations: Line style of left, top, right or bottom border. Array like for {@link SetLineStyle SetLineStyle}.</li> </ul> If a key is not present or is null, not draws the border. Default value: default line style (empty array).
        * <ul>
        ...
        * @param string  $style      Style of rendering. Possible values are: <ul> <li>D or empty string: Draw (default).</li> <li>F: Fill.</li> <li>DF or FD: Draw and fill.</li> <li>C: Draw close.</li> </ul> <ul> <li>D or empty string: Draw (default).</li> <li>F: Fill.</li> <li>DF or FD: Draw and fill.</li> <li>C: Draw close.</li> </ul>
        * <ul>
        *     <li>D or empty string: Draw (default).</li>
        *     <li>F: Fill.</li>
        *     <li>DF or FD: Draw and fill.</li>
        ...
        *     <li>DF or FD: Draw and fill.</li>
        * </ul>
        * @param array  $line_style Line style of polygon. Array with keys among the following: <ul> <li>all: Line style of all lines. Array like for {@link SetLineStyle SetLineStyle}.</li> <li>0 to ($np - 1): Line style of each line. Array like for {@link SetLineStyle SetLineStyle}.</li> </ul> If a key is not present or is null, not draws the line. Default value is default line style (empty array).
        * <ul>
        ...
        * @param string  $style                Style of rendering. Possible values are: <ul> <li>D or empty string: Draw (default).</li> <li>F: Fill.</li> <li>DF or FD: Draw and fill.</li> </ul> <ul> <li>D or empty string: Draw (default).</li> <li>F: Fill.</li> <li>DF or FD: Draw and fill.</li> </ul>
        * <ul>
        *     <li>D or empty string: Draw (default).</li>
        *     <li>F: Fill.</li>
        *     <li>DF or FD: Draw and fill.</li>
        * </ul>
        * @param array   $line_style           Line style of polygon sides. Array with keys among the following: <ul> <li>all: Line style of all sides. Array like for {@link SetLineStyle SetLineStyle}.</li> <li>0 to ($ns - 1): Line style of each side. Array like for {@link SetLineStyle SetLineStyle}.</li> </ul> If a key is not present or is null, not draws the side. Default value is default line style (empty array). <ul> <li>all: Line style of all sides. Array like for {@link SetLineStyle SetLineStyle}.</li> <li>0 to ($ns - 1): Line style of each side. Array like for {@link SetLineStyle SetLineStyle}.</li> </ul> If a key is not present or is null, not draws the side. Default value is default line style (empty array).
        * <ul>
        ...
        * @param array   $fill_color           Fill color. Format: array(red, green, blue). Default value: default color (empty array).
        * @param string  $circle_style         Style of rendering of inscribed circle (if draws). Possible values are: <ul> <li>D or empty string: Draw (default).</li> <li>F: Fill.</li> <li>DF or FD: Draw and fill.</li> </ul> <ul> <li>D or empty string: Draw (default).</li> <li>F: Fill.</li> <li>DF or FD: Draw and fill.</li> </ul>
        * <ul>
        *     <li>D or empty string: Draw (default).</li>
        *     <li>F: Fill.</li>
        *     <li>DF or FD: Draw and fill.</li>
        ...
        *     <li>DF or FD: Draw and fill.</li>
        * </ul>
        * @param array   $line_style           Line style of polygon sides. Array with keys among the following: <ul> <li>all: Line style of all sides. Array like for {@link SetLineStyle SetLineStyle}.</li> <li>0 to (n - 1): Line style of each side. Array like for {@link SetLineStyle SetLineStyle}.</li> </ul> If a key is not present or is null, not draws the side. Default value is default line style (empty array). <ul> <li>all: Line style of all sides. Array like for {@link SetLineStyle SetLineStyle}.</li> <li>0 to (n - 1): Line style of each side. Array like for {@link SetLineStyle SetLineStyle}.</li> </ul> If a key is not present or is null, not draws the side. Default value is default line style (empty array).
        * <ul>
        ...
        protected function utf8Bidi($ta, $forcertl=false) 
        {
            ...
            for ($i=0; $i < $numchars; $i++) {
                if ($ta[$i] == K_RLE) {
                    ...
                    if (count($remember)) {
                        ...
                        if (($remember[$last]['num'] == K_RLE)  
                            OR ($remember[$last]['num'] == K_LRE)  
                            OR ($remember[$last]['num'] == K_RLO)  
                            OR ($remember[$last]['num'] == K_LRO)
                        ) {
            ...
            for ($i=0; $i < $numchars; $i++) {
                if (($levcount > 0) AND (($i+1) < $numchars) AND ($chardata[($i+1)]['level'] == $prevlevel)) {
                    if (($chardata[$i]['type'] == 'N') AND ($chardata[($i-1)]['type'] == 'L') AND ($chardata[($i+1)]['type'] == 'L')) {
                        ...
                        AND (($chardata[($i-1)]['type'] == 'R') OR ($chardata[($i-1)]['type'] == 'EN') OR ($chardata[($i-1)]['type'] == 'AN')) 
                        AND (($chardata[($i+1)]['type'] == 'R') OR ($chardata[($i+1)]['type'] == 'EN') OR ($chardata[($i+1)]['type'] == 'AN'))
                    ) {
                        $chardata[$i]['type'] = 'R';
                    } elseif ($chardata[$i]['type'] == 'N') {
                    ...
                    if (($chardata[$i]['type'] == 'N') AND ($chardata[$i]['sor'] == 'L') AND ($chardata[($i+1)]['type'] == 'L')) {
                        ...
                        AND (($chardata[$i]['sor'] == 'R') OR ($chardata[$i]['sor'] == 'EN') OR ($chardata[$i]['sor'] == 'AN')) 
                        AND (($chardata[($i+1)]['type'] == 'R') OR ($chardata[($i+1)]['type'] == 'EN') OR ($chardata[($i+1)]['type'] == 'AN'))
                    ) {
                        $chardata[$i]['type'] = 'R';
                    } elseif ($chardata[$i]['type'] == 'N') {
                    ...
                    if (($chardata[$i]['type'] == 'N') AND ($chardata[($i-1)]['type'] == 'L') AND ($chardata[$i]['eor'] == 'L')) {
                        ...
                        AND (($chardata[($i-1)]['type'] == 'R') OR ($chardata[($i-1)]['type'] == 'EN') OR ($chardata[($i-1)]['type'] == 'AN')) 
                        AND (($chardata[$i]['eor'] == 'R') OR ($chardata[$i]['eor'] == 'EN') OR ($chardata[$i]['eor'] == 'AN'))
                    ) {
                        $chardata[$i]['type'] = 'R';
                    } elseif ($chardata[$i]['type'] == 'N') {
                        // N2. Any remaining neutrals take the embedding direction
            ...
            for ($i=0; $i < $numchars; $i++) {
                if (($chardata[$i]['type'] == 'B') OR ($chardata[$i]['type'] == 'S')) {
                    ...
                    $j = $i+1;
                    while ($j < $numchars) {
                        if ((($chardata[$j]['type'] == 'B') OR ($chardata[$j]['type'] == 'S')) 
                            OR (($j == ($numchars-1)) AND ($chardata[$j]['type'] == 'WS'))
                        ) {
                            $chardata[$i]['level'] = $pel;;
                            break;
            ...
            if ($arabic) {
                for ($i=0; $i < $numchars; $i++) {
                    if ($unicode[$chardata[$i]['char']] == 'AL') {
                        if (($i > 0) AND (($i+1) < $numchars)  
                            AND ($unicode[$chardata[($i-1)]['char']] == 'AL')  
                            AND ($unicode[$chardata[($i+1)]['char']] == 'AL') 
                            AND ($chardata[($i-1)]['type'] == $chardata[$i]['type']) 
                            AND ($chardata[($i+1)]['type'] == $chardata[$i]['type'])
                        ) {
                            // medial
